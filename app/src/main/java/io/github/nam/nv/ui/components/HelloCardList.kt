...
import io.github.nam.nv.setting.SettingKeyObject
import io.github.nam.nv.setting.dataStore
import io.github.nam.nv.ui.screen.MainScreenNavigationLinks
...
fun HelloCardList(
    ...
    Column(
        ...
        // 部分的な画面共有出来るよ
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE && dataStore.value?.contains(
                SettingKeyObject.IS_HIDE_HELLO_PARTIAL_MIRRORING_CARD) == false) {
            HelloPartialMirroringCard(
                modifier = Modifier.fillMaxWidth(),
                onClose = {
                    scope.launch {
